...
<img src="./assets/image-20250628160721144.png" alt="image-20250628160721144" style="zoom:67%;" />

<img src="./assets/image-20250628160730095.png" alt="image-20250628160730095" style="zoom:50%;" />

扫描线模板题，将给定的矩形左右对应的端点x提取出来并排序，因为给定是矩形，因此通过枚举相邻端点保证x区间内也是矩形，而不是类似于山脉数组的东西。

​	

```python
class Solution:
    def rectangleArea(self, nums) -> int:
        arr=[]
        for tmp in nums:        
            arr.extend([tmp[0],tmp[2]])
        ans=0
        mod=10**9+7
        arr=sorted(set(arr))
        for i in range(len(arr)-1):
            ...
            for x1,y1,x2,y2 in nums:
                if x1<=arr[i] and arr[i+1]<=x2:
                    tmp.append((y1,y2))
            if not tmp:continue
            # 由于不统计重复的面积，因此相当于要做一个区间合并
            tmp.sort()
            a,b=tmp[0]
            for l,r in tmp[1:]:
                if b>=l:
