...
## [删除元素后和的最小差值](https://leetcode.cn/problems/minimum-difference-in-sums-after-removal-of-elements/)

<img src="./assets/image-20250703165944243.png" alt="image-20250703165944243" style="zoom:67%;" />

<img src="./assets/image-20250703165953073.png" alt="image-20250703165953073" style="zoom:67%;" />

不论删除那些元素，最终的元素排列一定分为左右两部分，也就是前缀和后缀。根据题意知，两部分元素的个数一定是n，因此对每个前缀用堆维护最小的n个元素的和，对每个后缀维护最大的n个元素的和。
...
class Solution:
    def minimumDifference(self, nums) -> int:
        ...
        for i in range(3*n-1):
            if a1[i]!=inf and a2[i+1]!=inf:ans=min(ans,a1[i]-a2[i+1])
        return ans 
```
