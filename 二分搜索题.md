...
## [等和矩阵分割 II](https://leetcode.cn/problems/equal-sum-grid-partition-ii/)

![image-20250614160321694](./assets/image-20250614160321694.png)

<img src="./assets/image-20250614160328944.png" alt="image-20250614160328944" style="zoom:67%;" />

转换思想——考虑一个值后否存在合法的分割线。

假设删除了元素x，那么要找一条能等分上下两部分的水平分割线，分割线的位置具有单调性，随着分割线不断往下走，上方的元素值会越来愈大，下方的元素和会越来越小，因此二者的差会不断递减，所以可能通过二分找到是否存在一个分割。对于垂直方向有相同的操作。
...
class Solution:
    def canPartitionGrid(self, g: List[List[int]]) -> bool:
        m,n=len(g),len(g[0])
        # 前缀和求解
        pre=list(accumulate([sum(arr) for arr in g],initial=0))
        pre1=list(accumulate([sum(list(arr)) for arr in zip(*g)],initial=0))
        t=pre[-1]
        
        # 先判断不删除是否有合法的分割线
        for i in range(m):
            if pre[i+1]==pre[-1]-pre[i+1]:return True 
        for j in range(n):
            if pre1[j+1]==pre1[-1]-pre1[j+1]:return True 
            ...
            for j in range(n):
                ...
                while l<=r:
                    ...
                    # 找到后需要保证删除了不会使集合不连通
                        ...
                        # 如果是列向量，需要保证删除的位置在分割线左右，或者列首列尾
                        if n==1 and i and i!=mid and i!=mid+1 and i!=m-1:break
                        # 行数为2及以下，删除的位置只能在行的首尾
                        if not mid and not i and 0<j<n-1:break
                        if mid==m-2 and i==m-1 and 0<j<n-1:break
                        return True 
                    else:
                        l=mid+1
                # 垂直分割线
                ...
                while l<=r:
                    mid=(l+r)>>1
                    up,down=pre1[mid+1],(t-pre1[mid+1])
                    if mid>=j:up-=x
                    else:down-=x
                    ...
                    # 做相同的判断
                        ...
                        if m==1 and j!=mid and j!=mid+1 and j and j!=n-1:break 
                        if not mid and not j and 0<i<m-1:break
                        if mid==n-2 and j==n-1 and 0<i<m-1:break
                        return True 
                    else:
                        l=mid+1
        return False
...
## [面包店][https://www.acwing.com/problem/content/description/4908/]

<img src="./assets/image-20250314094450976.png" alt="image-20250314094450976" style="zoom:80%;" />



<img src="./assets/image-20250314094501399.png" alt="image-20250314094501399" style="zoom:67%;" />



一类特殊的二分，有两个操作x和y，但是二分不存在求解两个值的情况，这里的做法是二分sum=x+y，根据公式

<img src="./assets/image-20250314094744098.png" alt="image-20250314094744098" style="zoom: 67%;" />

遍历每个用例不断更新x的范围，最后如果x的范围合法那么sum是一个可行的操作。

<img src="./assets/image-20250314094850332.png" alt="image-20250314094850332" style="zoom: 67%;" />
...
for _ in range(R()):
    ...
    n,tc,tm=RR()
    nums=[]
    for _ in range(n):nums.append(RR())
    # 上下限
    l,r=0,tc+tm-2

    def cal(sum):
        # x的范围，sum次操作可以全给tm（不超过tm-1），如果有剩下的必须操作tc，sum次操作可以全部给自己 但是不能超过tc-1
        l,r=max(0,sum-(tm-1)),min(sum,tc-1)
        
        # 对于所有的顾客都要满足范围
        for i,j,c in nums:
            # c<=i*(tc-l)+(tm-(sum-l))*j
            # l*(j-i)<=(i*tc+tm*j-sum*j-c
            k=-(i*tc+tm*j-sum*j-c)
...
## [最大化游戏分数的最小值](https://leetcode.cn/problems/maximize-the-minimum-game-score/)

<img src="./assets/image-20250211105940603.png" alt="image-20250211105940603" style="zoom:67%;" />

<img src="./assets/image-20250211105946399.png" alt="image-20250211105946399" style="zoom:67%;" />

跳跃的方式有很多种难以考虑，贪心的想可以将所有的跳跃转换为两点之间的反复横跳来考虑(不然的话也没法做)
