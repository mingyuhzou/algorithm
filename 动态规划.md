...
定义f[i]\[j]表示前i个元素划分为j段时，可以得到的**最大能量值**，对于第i个元素有选或不选，如果选择则枚举左端点
...
显然对于给定的数据范围必须优化，对转移方程**f[l]\[j-1]+(pre[r]-pre[l]\*w)拆开有pre[r]\*w+f[l]\[j-1]-pre[l]*w**，因此定义mx[i]\[j]表示前i个数划分了j段时最大的f[l]\[j-1]-pre[l]*w。
...
class Solution:
    def maximumStrength(self, nums, k: int) -> int:
        ...
        f=[[-inf]*(k+1) for _ in range(n+1)]
        pre=list(accumulate(nums,initial=0))
        mx=[[-inf]*(k+1) for _ in range(n+1)]
        # 初始化
        for i in range(n+1):f[i][0]=mx[i][0]=0
        

        for i in range(1,n+1):
            # 划分的个数不能超过i，但其实超过了得到的结果也是-inf
            for j in range(1,min(i,k)+1):
                f[i][j]=max(f[i-1][j],pre[i]*cal(j)+mx[i-1][j-1])
            # 更新mx，注意是mx[i][j-1]而非nx[i][j]，因为从转移方程中是f[l][j-1]-pre[l]*w
            for j in range(1,k+1):
                # 最大值也可以来自于前一位
                mx[i][j-1]=max(mx[i-1][j-1],f[i][j-1]-cal(j)*pre[i])
        return f[n][k]
```
