...
就是扩展欧几里得算法中求解出的x或者是y的系数，另外根据费马小定理有：**当取模的数是质数的时候，a的逆元就是a^m-1**。

在对两个较大的数相除的结果取模的时候，同余定理对除法不起作用，根据费马小定理可以将除数转换为逆元后做乘法再取模，费马小定理要求模数是一个质数，除数=pow(除数,mod-2或-1,mod)。



如果频繁的求解逆元即使pow函数内部做了快速冥也非常耗时，这里可以预处理逆元
...
<img src="./assets/image-20240130160418078.png" alt="image-20240130160418078" style="zoom:50%;" />
...
<img src="./assets/image-20240130161336032.png" alt="image-20240130161336032" style="zoom:67%;" />

<img src="./assets/image-20240130161314099.png" alt="image-20240130161314099" style="zoom: 50%;" />
...
for i in range(2,n+1):
    ...
    for p in prime:
        if i*p>n:
            break
        is_prime[i*p]=False
        # 判断，如果已经包含过p了，那么i的欧拉公式中必然已经计算过(p-1)/p，所以只需补上N对应的项
        if i%p==0:
            res[i*p]=p*res[i]
            break
        res[i*p]=res[i]*(p-1)
...
对于不全为0任意整数a和b，令他们的最大公约数gcd(a,b)为g，那么任意的整数x和y都一定满足ax+by是g的倍数；特别的一定存在一组解，使得ax+by=gcd(a,b)成立
...
第二类斯特林数S(n,m)表示的是把n个**不同**的小球放在m个**互补区分的**的**非空**盒子里方案数，可以递推求解：**一个球独占一个盒子f(n-1,m-1)以及非独占f(n-1,m)*m**
...
在第二类斯特林数上有所变化，这里的盒子也就是歌曲的种类数是不同的，因此在状态转移时需要乘上可能。
...
数论分块可以在$O(\sqrt{n})$的时间复杂度内求出含有除法下取整的和式，例如$ \; \sum_{i=1}^n f(i) g\left(\left\lfloor \frac{n}{i} \right\rfloor\right)$，其中f(i)可以通过前缀和的方式求解出，利用n//i的结果是成块分布的来快速求解。

<img src="./assets/image-20250717203836422.png" alt="image-20250717203836422" style="zoom:67%;" />



伪码如下：

<img src="./assets/image-20250717203857801.png" alt="image-20250717203857801" style="zoom:67%;" />



## [Count A%B=C](https://atcoder.jp/contests/abc414/tasks/abc414_e)

<img src="./assets/image-20250717203934601.png" alt="image-20250717203934601" style="zoom:67%;" />
...
n堆物品，每堆有任意多个，两个玩家轮流取走任意一堆的大于0个物品。取走最后一个物品的人获胜
