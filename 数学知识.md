...
如果值得范围到了1e9，分解质因数：预处理出1e4.5范围内的质数(~3000)，在判断质因数时遍历质数表
...
对于长度为k的数组任取偶数个位置有$2^k-1$个中选法，奇数个位置也是同理。
...
从n数里取奇数个的方案数S1，取偶数个的方案数S2

由二项式定理：

$(1 + 1)^n = \sum_{k=0}^{n} \binom{n}{k} = 2^n = S_1 + S_2$
       $(1 - 1)^n = \sum_{k=0}^{n} (-1)^k \binom{n}{k} = 0 = S_1 - S_2$

两式联立得：

$S_1 = S_2 = \frac{2^n}{2} = 2^{n-1}$
