...
![image-20250504091935863](./assets/image-20250504091935863.png)

<img src="./assets/image-20250504092027143.png" alt="image-20250504092027143" style="zoom:67%;" />

将n排座位作为点，m作为点的值，gather操作查找maxRow范围内最小的大于等于k的值，scatter查找范围内的区间和是否大于等于k，如果大于等于就从第一个不为0的位置开始累减。上刷两个操作需要维护最大值和区间和，更新只需单点即可。
...
class BookMyShow:
    ...
    def __init__(self, n: int, m: int):
        self.n=n
        self.m=m
        self.mx=[0]*(2<<n.bit_length())        
        self.sum=[0]*(2<<n.bit_length())
        for i in range(n):
            self.update(1,0,n-1,i,m)
    ...
    # 查找范围内第一个大于等于x的位置
    def findMx(self,o,l,r,x,maxRow):
        # 如果区间的最大值小于x，那么一定不存在
        if self.mx[o]<x:
            return -1
        if l==r:
        ...
        mid=(l+r)>>1
        # 先查找左边，如果不存在则查找右边
        if (res:=self.findMx(o*2,l,mid,x,maxRow))<0:
            res=self.findMx(o*2+1,mid+1,r,x,maxRow)
        return res 
    
    def gather(self, k: int, maxRow: int):
        # 查找这个位置
        idx=self.findMx(1,0,self.n-1,k,maxRow)
        if idx<0:return []
        pre=self.query(1,0,self.n-1,idx,idx)
        s=self.m-pre
        # 更新这个位置
        self.update(1,0,self.n-1,idx,pre-k)
        return [idx,s]

    def scatter(self, k: int, maxRow: int):
        # 查找区间和是否能满足条件
        s=self.query(1,0,self.n-1,0,maxRow)
        if s<k:return False
        # 优化，从第一个还有座位的排开始
        idx=self.findMx(1,0,self.n-1,1,maxRow)
        # 累减
        while k:
            left=self.query(1,0,self.n-1,idx,idx)
            if left<=k:
                k-=left
                self.update(1,0,self.n-1,idx,0)
            else:
                self.update(1,0,self.n-1,idx,left-k)
                k=0
            idx+=1
        return True
```
