...
class SegmentTree:
    ...
    # 单点修改，将 idx 位置更新为 val，注意不需要修改懒标记
    def add(self, root, l, r, idx, val):
        if l == r:  # 到叶子节点
            self.tree[root] = val
            self.nums[idx] = val  # 同步修改原数组
            return
        mid = (l + r) >> 1
        if idx <= mid:
            self.add(root * 2 + 1, l, mid, idx, val)     # 去左子树
        else:
            self.add(root * 2 + 2, mid + 1, r, idx, val)  # 去右子树
        self.pushup(root)  # 更新
