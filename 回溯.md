...
回溯的复杂度计算通过树来形象化，以下述代码为例，这里树的高度为k-1，每层的节点数为factor[n]，总的时间复杂度就是${factor[n]}^{k-1}$。因此回溯中状态的选择至关重要

```python
def dfs(i,n,mx,mn):
            nonlocal min_ans,ans
            if i==k-1:
                tmp=max(mx,n)-min(mn,n)
                if tmp<min_ans:
                    min_ans=tmp
                    path[-1]=n
                    ans=path[:]
