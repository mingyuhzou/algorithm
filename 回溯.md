...
## [得到新鲜甜甜圈的最多组数](https://leetcode.cn/problems/maximum-number-of-groups-getting-fresh-donuts/)

<img src="./assets/image-20250318202107586.png" alt="image-20250318202107586" style="zoom:80%;" />

注意到batchsize的范围较小，将groups中的元素按照模n划分，贪心的思想，对于可以被batchsize整除的值，一定是开心的一组且不会影响到别的组，因此预先处理。

其他元素的选择会对其他元素造成影响，定义dp(cur,cnt)，cur表示前一个选完后的模数，cnt表示每一组还有多少人没有选，这里人数之和一共为30可以通过回溯加记忆化搜索解决。
        ...
        def dfs(cur,cnt):
            ...
            for i,x in enumerate(cnt,1):
                ...
                if x>0:
                    # 回溯
                    cnt[i-1]-=1
                    res=max(res,int(cur==0)+dfs((cur+i)%m,tuple(cnt)))
                    cnt[i-1]+=1
            return res 
        # 使用tuple记忆化搜索
...
![{10AA3295-7137-47B6-BBB9-8A3F26EBC1A8}](./assets/{10AA3295-7137-47B6-BBB9-8A3F26EBC1A8}.png)

<img src="./assets/{28699DB2-8E02-49F0-B15B-58EDD43DC3B8}.png" alt="{28699DB2-8E02-49F0-B15B-58EDD43DC3B8}" style="zoom:50%;" />

本题观察数据范围可以使用回溯解决，maxtime/time最多为10，那么路径的长度最多为10因此回溯树的高度不超过11，并且节点的边不超过4条那么树是一个四叉树，因此时间复杂度为4^k，其中k是maxtime/time，相当于在每一层有四种选择。
...
class Solution:
    def maximalPathQuality(self, values: List[int], edges: List[List[int]], maxTime: int) -> int:
        ...
        vis=[False]*n
        def dfs(root,t,c):
            # 回到起点，继续找，有总时间限制路径最长为10
            if not root:
                nonlocal ans 
                ans=max(ans,c)
            for nx,w in g[root]:
                # 超时了
                if w+t>maxTime:continue
                if vis[nx]:
                    dfs(nx,t+w,c)
                else:
                    # 记录答案
                    vis[nx]=True 
                    dfs(nx,t+w,c+values[nx])
                    vis[nx]=False
        vis[0]=True 
        dfs(0,0,values[0])
        return ans 
