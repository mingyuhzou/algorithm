...
**拓扑排序可以将环和其他的节点区分，拓扑序列不唯一，序列中边起点一定排在终点之前。**
...
<img src="assets/image-20240207114551966.png" alt="image-20240207114551966" style="zoom: 80%;" />



这里并非要找到拓扑序，只是借助拓扑序来保证先修课的关系，在排序过程中要枚举其他节点通过递推关系构建枚举的节点是否与下一个节点满足先修课，以此构建出所有节点之间是否满足先修课。
...
给定的两个数组规定了行和列元素摆放的相对顺序，但是这个顺序不是很明确，可以用拓扑排序给出所有点的顺序。
...
难点在于同一组的项目要相邻，不同组的项目之间存在依赖关系，这里做**两次拓扑排序**，第一次按照组排好序即将项目之间的依赖映射为组之间的依赖，然后拓扑排序每个项目，将项目按顺序放入到组中。
...
<img src="./assets/{98D361B7-1D76-476A-9AF6-0EAAB234FB2B}.png" alt="{98D361B7-1D76-476A-9AF6-0EAAB234FB2B}" style="zoom:67%;" />
...
对于有多个状态的问题，需要扩展**dis**数组，类似DP的做法。

如果要同时求解所有点到其他点的最小距离，**dis数组都设置为inf**，再将**所有的点入队做Dijkstra**

注意如果边权为**0或1**，那么要用**0-1BFS**而不是Dijkstra(会超时)
...
用Dijkstra找最短路的过程中使用dp求解路径数，定义dp[i]为0~i最短路的数目，如果存在d+v<dis[k]说明从0到x再到y的路径是目前最短的，所以更新f[y]为f[x]，如果等于，说明从0到x再到y的路径与之前找到的路径一样短，所以把f[y]增加f[x]。
...
本题有两个约束条件一个是费用一个是时间，从起点出发第一次到达的点的费用一定是最低的，**因为本题中权重是在点上的而非边上。相当于固定了一个变量去找另一个变量，即已经确定一个先走过的位置费用一定更小，如果时间不会更小的话那么一定是无用的状态，也就是一个费用更大但时间更短的是一个合法的状态**，为了固定费用根堆以费用最为权重。
...
<img src="./assets/image-20250509190224472.png" alt="image-20250509190224472" style="zoom:67%;" />
...
选择遍历给定的两个字符串，如果对应位置两个字符串相同，可以选择跳过也可以选择就在这个位置替换;替换子串多长?如果选择枚举给定字符串的各个位置，由于给定字符串的长度最多为1000，很容易超时，因此转向枚举给定的用于替换的字符数组中所有字符串的长度（最多只有100个不同的长度），需要对字符数组按照长度进行分组。能不能替换？要看两个子串是否在字符数组中。
...
## [Sightseeing Tour](https://atcoder.jp/contests/abc369/tasks/abc369_e)

![image-20250509190745207](./assets/image-20250509190745207.png)

<img src="./assets/image-20250509190756681.png" alt="image-20250509190756681" style="zoom:50%;" />
...
如果权重定义为**一条路径上or一个子图中**的最大或其他性质，那么需要用到MST求解。
...
## [Sum of Max Matching](https://atcoder.jp/contests/abc383/tasks/abc383_e)

![image-20250218224614297](./assets/image-20250218224614297.png)

![image-20250218224628955](./assets/image-20250218224628955.png)
...
这里使用两个数组start和end记录A和B中每个元素出现的次数，在使用Kruskal的过程中，当一条边可以连接两个集合中的数时，这条边的权值就是两个集合中点相互到达的路径上的最小的最大值，因为从小到大枚举边。注意这里可能会连接集合内部的点而不是两个集合之间，因此最后需要修改start和end
...
for a,b,w in edges:
    a-=1
    b-=1
    # 分别找出边的端点对应的集合，端点在之前可能与序列内部其他的点相互连接形成了一个小的连通块
    fa,fb=uf.find(a),uf.find(b)
    # 已经连接，跳过
    if fa==fb:continue
    # 连接，这里是将fa连到fb上，因为如果端点是在集合内，则默认以b为根，find不会改变被连接者的根
    uf.union(fa,fb)
    # a在start中,b在end中，取最小
...
将DAG转换为SCC所需添加的边的个数是**max(入度为0的点的数目，出度为0的点的数目)**
...
假设形成的图是一个有向无环图，如果存在两个及以上出度为0的点(即终点)，那么一定无解。

但是如果图中存在环的话就会出现如下的情况，需要通过缩点防止漏判端点为环的情况

<img src="./assets/image-20250509195229402.png" alt="image-20250509195229402" style="zoom:50%;" />
...
第二问要求将图变为强联通分量也就是图中所有的点都可以相互到达，那么有结论需要连接的边数是max(入度为0的点的数目，出度为0的点的数目)。
...
x->y是桥有**low[y]>dfn[x] 不能等于**，这样就不会回溯到比x更早的时间点，那么删除x->y就会导致不连通。
...
![{071D17F8-1196-42F9-AE69-AB57E1FD988B}](./assets/{071D17F8-1196-42F9-AE69-AB57E1FD988B}.png)

<img src="./assets/{0F34E932-A08F-4500-A5A8-BB652F3BBEF0}.png" alt="{0F34E932-A08F-4500-A5A8-BB652F3BBEF0}" style="zoom:67%;" />



从特殊开始思考，如果无向图中没有环，那么就是一颗树，通过枚举每个点做bfs得到的最大的深度就是最多的分组。

如果图中存在环，如果是奇环那么一定无解，如果是偶数则可以分为两部分，因此要判断图中是否只有偶数环(通过红蓝染色法)
...
## [Tree Game](https://atcoder.jp/contests/abc398/tasks/abc398_e)

![image-20250327220300145](./assets/image-20250327220300145.png)

<img src="./assets/image-20250327220421646.png" alt="image-20250327220421646" style="zoom:67%;" />

连接两个点不会出现奇环，**那么一定是连接处于二分图左右两部分的点**，由于给定的数据范围不大，因此可以构建所有可行的边然后遍历。
...
# 两组点
red=[i  for i,x in enumerate(color) if x==0]
blue=[i  for i,x in enumerate(color) if x==1]
edges=[(i,j) for i in red for j in blue]
...
if total&1:
    ...
    while (min(edges[idx][0],edges[idx][1]),max(edges[idx][0],edges[idx][1])) in vis:
        idx+=1
    l,r=edges[idx]
    print(min(l,r)+1,max(l,r)+1,flush=True)
    vis.add((min(l,r),max(l,r)))
else:
...
二分图最小点覆盖——**选择最少的点满足每条边都有一个点被选择**

**Konig定理**：二分图最小点覆盖等于二分图最大匹配



二分图最大独立集——**选出最多的点**，满足二者之间没有边相连
...
### [使矩阵中的 1 互不相邻的最小操作数](https://leetcode.cn/problems/minimum-operations-to-remove-adjacent-ones-in-matrix/)

![{895EBAD3-CAE0-408B-8319-A248B21DDEAE}](./assets/{895EBAD3-CAE0-408B-8319-A248B21DDEAE}.png)

矩阵中的点按照**坐标和的奇偶**可以划分为两组，而**相邻的两点一定属于不同的组**，因此将所有为1的坐标点与相邻的坐标点相连可以得到一个二分图，题目要求删除最少的点使得矩阵独立，**那么就是删除最少的点覆盖二分图所有的边**，就是找出二分图中最小点覆盖(**选出最少的点使得所有的点都被选中，如果边的一个端点被选中那么边也算是被选中**)，**根据Konig定理有二分图最小点覆盖等于二分图最大匹配**
        ...
        def find(i):
            for nx in g[i]:
                ...
                if nx not in st:
                    st.add(nx)
                    # 没有男朋友或者可以换一个
                    if nx not in link or find(link[nx]):
                        link[i]=nx
                        link[nx]=i
                        return True 
            return False
...
1. 对每个节点按照(入度，节点标号大小)排序
2. 对每个节点构架A，A[i]表示与节点i相连且排在i之后的节点有那些
3. 遍历每一条边，对边的两个端点对应的A取并集，累加并集的大小，最后的结果就是不重复的三元环个数

![{CE082EB3-79DE-46DE-A412-7B18226A69D4}](./assets/{CE082EB3-79DE-46DE-A412-7B18226A69D4}.png)

<img src="./assets/{93C4B24F-34DD-4AD5-8285-CAC391161399}.png" alt="{93C4B24F-34DD-4AD5-8285-CAC391161399}" style="zoom:67%;" />
...
或者对于给定的数据范围，对每个点暴力枚举相邻的两个节点，查看这两个节点是否相连。
