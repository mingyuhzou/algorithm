...
<img src="./assets/image-20240711093927993-1732668304387-9.png" alt="image-20240711093927993" style="zoom:67%;" />

<img src="./assets/image-20250609183137661.png" alt="image-20250609183137661" style="zoom:67%;" />

枚举每一个元素作为山脉数组的顶时，最大的高度和，前后缀分解使用栈维护求解。

当nums[i]<=nums[i+1]时，为了得到最大的和，一定是累加nums[i+1]，反之为了保证i+1作为顶点就需要删除前面的大于它的点同时删除该点贡献的累和
    ...
    def maximumSumOfHeights(self, nums: List[int]) -> int:
        ...
        for i,v in enumerate(nums):
            ...
            while len(d)>1 and nums[d[-1]]>v:
                j=d.pop()
                # 通过坐标相减可以得出这个位置之前栈顶的元素有多少个，因为对于当前的栈顶元素他前面一定是已经修改过的小于等于它的
                temp-=nums[j]*(j-d[-1])
                temp+=v*(j-d[-1])
            d.append(i)
            pre.append(temp)
...
<img src="./assets/image-20250628170438083.png" alt="image-20250628170438083" style="zoom:67%;" />

显而易见的要找和至少为k的子数组，必然要使用前缀和找出pre-?>=k的位置，在已遍历的位置中，?对应的值前缀值一定是越小越好，且尽可能靠右，那么一旦来了一个新的值并且这个值也很小，前面的比它大的一定就是无用的状态需要丢弃，因此需要维护一个单调递增的序列，这需要用到队列，题目要求长度最小，因此在维护的队列中二分找出第一个满足条件的位置。
...
class Solution:
    def shortestSubarray(self, nums, k: int) -> int:
        ...
        for i in range(n):
            x=pre[i+1]
            # 弹出无用值，左侧更大于等于的值一定不如当前值好
            while d and d[-1][0]>=x:
                d.pop()

            d.append((x,i))
            #二分找到第一个(最靠右)满足条件的位置
            idx=bisect_left(d,(x-k+1,-1))-1
            if idx>=0:
                ans=min(ans,i-d[idx][1])
...
## [计数质数间隔平衡子数组](https://leetcode.cn/problems/count-prime-gap-balanced-subarrays/)

![image-20250715191555395](./assets/image-20250715191555395.png)

<img src="./assets/image-20250715191559952.png" alt="image-20250715191559952" style="zoom:67%;" />

可以很自然地想到要用队列维护最大的质数和最小的质数，关键点在于题目要求最大质数于最小质数的差小于等于K，那么子数组计数固定右端点再用滑动窗口找到左端点的最左位置，那么左端点的区间就确定了，因为在固定了右端点的情况下，大的数组都能满足条件(大的数组的最大值与最小值)，那么收缩后也一定满足。

对于题目要求的至少有两个质数条件，只需维护遍历到的倒数第二个质数即可。
...
**枚举每个矩形以其上边界位基准求解最大的正方形**，以该上边形成的最大矩形取决于两边比它小的上边的位置，使用单调栈找出两边小于它的位置即可。
...
<img src="./assets/image-20240324223331221.png" alt="image-20240324223331221" style="zoom: 80%;" />
...
<img src="./assets/image-20250609185121605.png" alt="image-20250609185121605" style="zoom:67%;" />
...
<img src="./assets/image-20250609185710043.png" alt="image-20250609185710043" style="zoom:80%;" />
