...
每对坐标的乘积是一个完全平方数，对于满足该条件的坐标，将其中中的完全平方数剔除掉也仍然满足条件，因此把所有的坐标剔除其中的完全平方数得到core，只有core相等的一组数才能满足每对之间可以满足条件。
...
**将N+1个球1放到N个抽屉里，至少有一个抽屉会有两个(或以上)的球**
...
当k为偶数时，只能做到修改偶数个位置，那么问题变为从n个位置选偶数个位置有多少选法，结果就是1<<(n-1)，k为奇数也可以看作是从n个位置中选奇数个和偶数个的选法。
