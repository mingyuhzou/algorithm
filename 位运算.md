...
![image-20250414223536608](./assets/image-20250414223536608.png)

对异或区间和结论的应用

![image-20250414223704335](./assets/image-20250414223704335.png)

遍历异或前缀和数组pre，考虑每一位的贡献，当pre[i]的j位置是1时它所做的贡献就是，遍历到的位置只会与前面的位置计算因此不重不漏。

```python
(i*cnt0(前面该位上为0的个数)-sum0(前面该位为0的下标和))*(1<<j) # 左侧实际上对应的是公式中的(r-l+1)
```
...
显而易见的从高位开始贪心一直走到低位，如果高位允许设置为1那么一定要选择。这里的关键在于，最后不要剩下糖果否则是不合法的，因此对于不能设置为1的位不能直接全填为0，需要判断剩下的糖果是否能满足小于等于01111*m，这样才能保证最后糖果为0，如果不满足的话，那么寻找最小的k使得n-k\*(1<<bit)<=01111\*m
