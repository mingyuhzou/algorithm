...
通过lowbit的大小可以求解一个数中有多少为2质数，即lowbit所对的幂次
...
这里通过刷表法求解：对于i位置，枚举0到$2^i-1$的集合，它们不包括i位置的元素，将i添加到这些集合中求解新的结果。

```python
# 需要预处理子集的xor和and值
sub_xor=[0]*(1<<len(nums))
sub_and=[0]*(1<<len(nums))
sub_and[0]=-1
# 递推求解
for i,x in enumerate(nums):
    high_bit=1<<i
    for j in range(high_bit):
        sub_xor[j|(1<<i)]=sub_xor[j]^x
        sub_and[j|(1<<i)]=sub_and[j]&x
        sub_and[0]=0
```

## 集合所对的最大值

一个集合所对的最大值一般认为要从子集退出，而一个集合所对的子集个数是2的幂次级别的，如果通过DP转移是不可能实现的。实际上，只要枚举集合中所有的设置位即可，因为各个子集所对的最大值实际上会被集合去掉设置位后的子集包含。



<img src="./assets/image-20250902162912030.png" alt="image-20250902162912030" style="zoom:67%;" />

<img src="./assets/image-20250902162919037.png" alt="image-20250902162919037" style="zoom:67%;" />

对于本题只要求出每个X的补集所对的最大值即求解。
...
class Solution:
    def maxProduct(self, nums: List[int]) -> int:
        ...
        for s in range(1<<n):
            t=s
            # 不需要枚举每一位，只要找到设置位即可，因此可以使用lowbit
            while t:
                lb=t&-t
                tmp=f[s^lb]
                if tmp>f[s]:
...
![image-20250520164850899](./assets/image-20250520164850899.png)

异或中x^x=0，那么令数组和为s，异或值为x，那么先将s变为0方便构造，即再添加一个s，那么此时数组的和等于s+x，为了得到两倍，数组再添加一个s+x，根据0异或上任何数都是这个数的性质，最终只需操作两次。
...
## [异或计算](https://oj.redrock.team/problem/2055)

![image-20250324215835159](./assets/image-20250324215835159.png)

异或运算基本思想——拆位

对于给定的数据范围分别考虑每一位的贡献，而每一位的贡献需要用DP求解，当枚举p位时，令f0(i)表示选了前i个此时该位出现次数为偶数的方案数，f1反之，也就是状态机DP。
...
<img src="./assets/image-20240719112428297-1733840243519-161.png" alt="image-20240719112428297" style="zoom: 80%;" />
...
每一位的贡献就是这一位上能组成多少个1(因为是**两两组合**)，假如说由两个0和三个1那么这个位置能异或出6个1，对结果的贡献就是(2^i)*6，最后加上数组的和(**区间为1的情况**)



## [Sum of XOR Functions](https://codeforces.com/problemset/problem/1879/D)

<img src="./assets/image-20250911141932459.png" alt="image-20250911141932459" style="zoom:67%;" />

对异或区间和结论的应用

![image-20250414223704335](./assets/image-20250414223704335.png)

遍历异或前缀和数组pre，考虑以每个元素值为结尾的贡献值(拆位计算)，当值的j位置是1时它所做的贡献就是

```python
# i*cnt0因为以这一位结尾能贡献的有cnt0个，对应r；
(i*cnt0(前面该位上为0的个数)-sum0(前面该位为0的下标和))*(1<<j) 
```
...
for i in range(31):
    ...
    for idx,s in enumerate(pre):
        if s>>i&1:
            ans+=(1<<i)*(cnt0*idx-sum0)
            cnt1+=1
            sum1+=idx
        else:
            ans+=(1<<i)*(cnt1*idx-sum1)
            cnt0+=1
            sum0+=idx
        ans%=mod
print(ans)
...
## [异或运算](https://oj.redrock.team/contest/13/problem/2)

![image-20250414224357151](./assets/image-20250414224357151.png)

观察公式，求解所有数任意组合进行异或运算的和，这里求解对于每一个二进制位有多少种有效位的组合方式，那么最后的结果就是有效位的组合方式数*(1<<j)的累和。

定义cnt0[i]表示已经选了I个元素，该位上和为偶数的方案数有多少种，cnt1[i]同理，由于E是任意数组合，因此第i个元素可以选任意数，那么状态机DP的转移方程就是

![image-20250415191612176](./assets/image-20250415191612176.png)
...
## [Minimum OR Path](https://atcoder.jp/contests/abc408/tasks/abc408_e)

![image-20250607182231543](./assets/image-20250607182231543.png)

<img src="./assets/image-20250607182241813.png" alt="image-20250607182241813" style="zoom:67%;" />
...
## [汉堡猪猪分糖果](https://ac.nowcoder.com/acm/contest/112544/F)

<img src="./assets/image-20250707220428372.png" alt="image-20250707220428372" style="zoom:67%;" />



显而易见的从高位开始贪心一直走到低位，如果高位允许设置为1那么一定要选择。这里的关键在于，最后不能有剩下的糖果否则是不合法的，因此对于不能设置为1的位不能直接全填为0，需要判断剩下的糖果是否能满足小于等于01111*m，这样才能保证最后糖果为0，如果不满足的话，那么寻找最小的k使得n-k\*(1<<bit)<=01111\*m
...
## [小苯的xor图](https://ac.nowcoder.com/acm/contest/115861/E)

<img src="./assets/image-20250829093236335.png" alt="image-20250829093236335" style="zoom:67%;" />

异或运算考虑拆位，这里的简单路径长度为2，那么只需枚举每条边求解异或值，然后看看于边的端点相连的在每个比特位上有多少0和1，直接求解即可。这里注意不要预先处理出每条边相邻的01个数，要一边遍历一遍求解，这里可以保证不重不漏。
...
# 每个点在每个比特位上对应的1的个数
memo=[[0]*31 for _ in range(n+1)]
ans=0
# 每个点相邻的点有多少个，用于求解0的个数
cnt=defaultdict(int)


def slove(a,b):
    global ans
    xor_=nums[a-1]^nums[b-1]
    # 拆位
    for i in range(31):
        if xor_>>i&1:
            ans+=(cnt[a]-memo[a][i])<<i
        else:
            ans+=memo[a][i]<<i
        # 更新
        memo[a][i]+=nums[b-1]>>i&1
        ans%=mod
    # 记录个数
    cnt[a]+=1
...
<img src="./assets/image-20231227221706798-1733840243519-173.png" alt="image-20231227221706798" style="zoom: 67%;" />

<img src="./assets/image-20231227221713494-1733840243519-174.png" alt="image-20231227221713494" style="zoom: 67%;" />

题意为：找出以每个元素为起始位置的子数组中，按位或能得到最大值的最短子数组。

或运算得到的子数组只会是递增或是不变的具有单调性，使用一个数组来维护每个位置的元素所能得到的最大值以及这个位置，百万级别的数据对应到二进制上最多只有三十几个状态，因此维护的数据不会太大，而每个元素能对应的区间又可以由前一个元素推出。
...
class Solution:
    def smallestSubarrays(self, nums: List[int]) -> List[int]:
        n=len(nums)
        ans=[0]*n
	# 维护状态（或值，右端点索引），每次加到尾部，因此数组首部就是最大的异或值
        ors=[]
        # 倒序遍历，因为要以i为起点
        for i in range(n-1,-1,-1):
            ors.append([0,i])
            ...
            while r<len(ors):
                ...
                r+=1
            del ors[l+1:]
            # 数组的第一个元素对应按位或最大的子数组，在去重后能保证对于或值相同的，只会保留最短的那个
            ans[i]=ors[0][1]-i+1
        return ans     
```
...
最后是如何判断从其起始元素到区间中的某一个位置是否满足条件：利用一个前缀和数组快速判断，假设这个位置是x，那么子数组的累和就是pre[x+1]-pre[i]，`条件是pre[x+1]-pre[i]==p[0] (当前值)，这里不用一个个遍历x，求解pre[i]+p[0]的值是否在pre数组中即可`。
...
<img src="./assets/image-20240516135346203-1733840243519-181.png" alt="image-20240516135346203" style="zoom: 80%;" />



观察发现，只要有1就可以把所有的数变为1，且最小的结果是不为1的个数（与1求最大公约数的结果是1）.

或者可能是一段子数组相互求最大公约数得到1，然后用这个1与其他的数运算即可，对于给定的数据范围可以暴力枚举出所有的子数组找到最小的满足最大公约数是1的，这表示这一段子数组同通过操作m-1次相互求最大公约数得到了1，再用这个1和其他的求解。
...
<img src="./assets/image-20240826094801759-1733840243519-168.png" alt="image-20240826094801759" style="zoom:67%;" />
...
位运算经典技巧，本质上是对每一位去构造答案

## [给定操作次数内使剩余元素的或值最小](https://leetcode.cn/problems/minimize-or-of-remaining-elements-using-operations/)

![image-20240526180835627](./assets/image-20240526180835627-1733840243519-169.png)	   											

<img src="./assets/image-20240526180840757-1733840243519-170.png" alt="image-20240526180840757"  />
...
for i in range(31,-1,-1):
    ...
    for l,r in nums:
        # 取出价值对应的位数
        r&=mask
        # 如果不会使答案变小，可以加入，把对应的所有体积都添加，因为体积与运算一定会变小所以不用判断直接加入
        if ans&r==ans:
            ans_res&=l
    # 最后体积不满足要求
    if ans_res>k:
...
用于从一组数中选出异或值最大的子集，其基本思想是从数组中选出一组基(类似于线性代数中的极大无关组，可以表示出其他任何值)，利用这组基求解出最大的异或值。
...
class XorBasis:
    ...
    def insert(self,x):
        ...
        for i in range(self.n-1,-1,-1):
            if x>>i&1:
                # 保证最高位不冲突，即和之前的基是线性无关的
                if not b[i]:
                    b[i]=x# 新增一个基
                    return
                x^=b[i]# 保证每组基的最高位不冲突，便于后续求解
            # 循环正常结束，说明值可以被已有的基表出
    def max_xor(self):
        res=0
        b=self.b
...
## [划分数组得到最大异或运算和与运算之和](https://leetcode.cn/problems/partition-array-for-maximum-xor-and-and/)

​			<img src="./assets/image-20250818170357650.png" alt="image-20250818170357650" style="zoom:67%;" />	

<img src="./assets/image-20250818170404122.png" alt="image-20250818170404122" style="zoom:50%;" />

观察式子，这里的and求解比较突兀，考虑到数据范围可以直接枚举用于and求和的子集，接下来问题变化从划分两个子集得到最大的异或和。根据异或运算的性质，如果一个bit位上有奇数个1，那么无论怎么划分一定会存在一个1，称这些位置为j；如果是偶数，那么可以划分为偶数+偶数|奇数+奇数，虽然得到的结果不一样，但是可以发现不论是全奇还是全偶，在这一位上都有XOR(A)==XOR(C)，因此可以将剩余部分的公式化简为：

![image-20250818170904420](./assets/image-20250818170904420.png)

其中XOR(S)可以理解为j的贡献，A\`则是剩余子集去除了j这些位置，那么最终问题变为：如何从A\`从选出一个异或值最大的子集，这里用到线性基。
