...
class UF:
    def __init__(self,n):
        self.parent=[*range(n)]
        self.size=[1]*n
    # 可以保证如果u是连通块第一个进行合并的点，那么u一定是整个连通块的根节点
    def union(self,u,v):
...
并查集的一个用处是**判断图中环的个数**——遍历每一条边，如果边的两个端点已经是一个集合块中了则说明有一个环。

并查集可用于找下一个位置，比如 1 2 3，想要1的右侧是3也就是跳过2，那么将2位置的**parent**设置为3，那么在访问2位置时通过**find(2)**可以跳到3位置。

如果并查集中加入了**不存在于数组中**的元素，那么计数时使用**size**会得到错误的结果，正确的方法应该是对每个数组中的每个数的**根**进行计数。[按公因数计算最大组件大小](https://leetcode.cn/problems/largest-component-size-by-common-factor/)

注意并查集板子1.0有问题，在连接时应该是p->q上，而不是判断谁的size更大
...
## [数组的最大公因数排序](https://leetcode.cn/problems/gcd-sort-of-an-array/)

<img src="./assets/image-20250506092733199.png" alt="image-20250506092733199" style="zoom: 50%;" />

<img src="./assets/image-20250506092744373.png" alt="image-20250506092744373" style="zoom:67%;" />

拥有相同质因数的值可以相互交换，可以认为是处于同一个连通块，一个连通块中的值可以相互交换，因此将原数组与排序后的数组一一比较，查看二者是否处于同一连通块。
...
class Solution:
    def gcdSort(self, nums: List[int]) -> bool:
        ...
        for i,x in enumerate(nums):
            ...
            while x>1:
                # 对质数做偏移，防止与数值混淆
                uf.union(tmp,bisect_left(prime,min_fac[x])+n)
                x//=min_fac[x]
        # 看看两个位置上的值是否属于一个连通块
        for i,j in zip(sorted(nums),nums):
...
<img src="./assets/image-20250506105959133.png" alt="image-20250506105959133" style="zoom: 67%;" />

<img src="./assets/image-20250506110010498.png" alt="image-20250506110010498" style="zoom:67%;" />



与上一题一样，但是这里要求解连通块的大小，首先处理出所有的因数，然后将数组中的数组与因数相连，这里做不做偏移都一样，因为问题在于，并查集中加入了**不存在于数组中的数字**(因数)所以会导致计算连通块大小错误，这里的技巧是使用代表元计算(即union(x,y)中的y)，对于数组中的每个值，如果它们同属于一个连通块，那么根一定是一样的，直接对根进行计数，得到的结果就是连通块的大小。
